...
describe('VersionPreviewModal', () => {
  ...
  describe('version content display', () => {
    ...
    it('displays read-only indicator', () => {
      expect(wrapper.find('.read-only-indicator').exists()).toBe(true)
      expect(wrapper.find('.read-only-indicator').text()).toContain('Read-only preview')
    })
  ...
  describe('close functionality', () => {
    ...
    it('emits close event when clicking Close button in footer', async () => {
      const closeButton = wrapper.findAll('.btn-secondary')[0]
  ...
  describe('restore functionality for owner', () => {
    ...
    it('shows loading state during restore', async () => {
      ...
      expect(wrapper.find('[data-testid="confirm-restore-button"]').text()).toContain('Restoring...')
      expect(wrapper.find('[data-testid="confirm-restore-button"]').attributes('disabled')).toBeDefined()
    })
